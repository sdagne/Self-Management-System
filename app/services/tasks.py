...
def send_telegram_reminder(self, ticket_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        send_telegram_notification.delay(ticket.telegram_chat_id, message, ticket.ticket_number)
...
def cleanup_expired_tickets(self) -> Dict[str, Any]:
    ...
    try:
        ...
        expired_tickets = (
            self.db.query(Ticket)
            .filter(Ticket.status == TicketStatus.WAITING, Ticket.created_at < expiry_threshold)
            .all()
        )
...
def generate_daily_report(self) -> Dict[str, Any]:
    ...
    try:
        ...
        # Collect statistics
        total_tickets = (
            self.db.query(func.count(Ticket.id)).filter(Ticket.created_at >= today_start).scalar()
        )

        completed_tickets = (
            self.db.query(func.count(Ticket.id))
            .filter(Ticket.created_at >= today_start, Ticket.status == "completed")
            .scalar()
        )

        avg_wait_time = (
            self.db.query(
                func.avg(func.extract("epoch", Ticket.updated_at - Ticket.created_at) / 60)
            )
            .filter(Ticket.created_at >= today_start, Ticket.status == "completed")
            .scalar()
            or 0
